...
onMounted(
	async () => {
		...
		const dataPieChart = {
			...
			datasets: [{
				label: '',
				data: [statisticsStore.statistics?.all.total_of_orders_by_type[0].delivered,
		...
		<VCol md="12" sm="12">
			<VCard>
				<v-card-title align="center">
					Best Selling Products By Type</v-card-title>
				<VRow>
					<VCol v-for="item in statisticsStore.statistics?.all.best_selling_products_by_type" cols="12"
						lg="3" sm="4">
						<div v-for="product in item">
							<VCard class="h-100e" align="center">
